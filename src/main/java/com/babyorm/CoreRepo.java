...
public abstract class CoreRepo<T> {
    ...
    protected void init(Class<T> entityType) {
        ...
        this.entityMapper = new EntityMapper<>(fields, fieldNameToColName);
    ...
    public T insert(T record) {
        ...
        try (Connection conn = getConnection()) {
            ...
            if (!hasKey && this.isAutoGen) {
                ...
                lookupKeyProvider = keyFields.stream().collect(Collectors.toMap(Field::getName, f-> () -> entityMapper.getResultValueByPosition(f, keys, 1)));
            } else {
    ...
    private LinkedHashMap<String, ?> toKey(Map<String, KeyProvider> keyProviders){
        return keysToColumnNames(
                keyProviders.entrySet().stream().collect(
